...
function initializeBooklet() {
    ...
    $(".clickimageimg").on("click", clickImageTapHandler);
}
...
function clickImageTapHandler(event) {
    ...
    for (pos in specs) {
        ...
        if (tapXratio > minx && tapXratio < maxx && tapYratio > miny
            ...
            audio.play();
            console.info(event);
            console.info(event.clientX - $("#sparkle").width() / 2 + "  " + event.clientY - $("#sparkle").height());
            //$("#sparkle").css({'left': event.clientX - $("#sparkle").width() / 2+"px", 'top': event.clientY - $("#sparkle").height() / 2+"px"});
            $("#sparkle").css('left',tapx+$("#sparkle").width() / 2);
            $("#sparkle").css('top',tapy+$("#sparkle").height() / 2);
            $("#sparkle").show();
            $.wait(500).then(function() {
                $("#sparkle").hide();
            });
            console.info($("#sparkle").css('left')+"  "+$("#sparkle").css('top'));
        }
    }
...
function previewBookPage(page, pageno, gotopage) {
    ...
    if (page['page_type'] == "CONTENT_PAGE") {
        ...
        var div2='<div id="sparkle" style="display:none;position:absolute; z-index: 9999;"><img src="'+window.baseurl+'/assets/images/sparkle.png" width="50px" height="50px" /></div>';
        div = '<div title="' + page['page_type'] + '" class="imagepage"><img class="clickimageimg" data-specs=\'' + page['body'] + '\' width="80%" src="' + imagebasepath + page['image'] + '"/>'+div2+'</div>';
        addpage(div+div2, pageno);
        $(".clickimageimg").on("click", clickImageTapHandler);
